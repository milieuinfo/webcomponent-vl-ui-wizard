...
describe('vl-wizard', async () => {
    ...
    it('als gebruiker kan ik alleen door de panelen van de wizard navigeren wanneer ik de checkbox aanzet', async () => {
        ...
        let checkbox = await content.findElement(By.css('vl-checkbox'));
        ...
        checkbox = await content.findElement(By.css('vl-checkbox'));
        await checkbox.click();
        await wizard.previous();
        activePane = await wizard.getActivePane();
        await assert.eventually.isTrue(activePane.equals(pane1));
